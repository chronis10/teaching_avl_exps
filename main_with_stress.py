...
from external_com.tr_interfaces.reciever_transmitter_interfaces import (
    Transmitter,
    Reciever,
)
...
def predictor_service(
    ...
    from ai_module.stress import StressModel

    subj_id = None  # TODO: ADD HERE SUBJECT ID
    per_model = Personalization_Model(
        drivers_profiles[f"driver{current_driver}"]["model_path"]
    )
    stress_model = StressModel(
        subj_id,
        {
            ...
            "online": True,
            "baseline_path": f"baselines/{subj_id}.csv",
            "buffer_size": 1000,
        },
    )
    transmiter.send({"handshake": 0, "timestamp": time.time()})
    ...
    while True:
        ...
        if data is not None:
            print(f"Incoming data: {data}")
            try:
                stress_value = stress_model.prediction(deepcopy(data))
                prediction = per_model.prediction(data)

                packet = {"prediction": prediction, "timestamp": time.time()}
                if action_logger is not None:
                    to_log = deepcopy(packet)
                    to_log["stress"] = stress_value
                    action_logger.log(to_log)
                transmiter.send(packet)
                print(f"Transimited: {packet}")
            except Exception as error_message:
                print(error_message)
...
if __name__ == "__main__":
    ...
    argParser.add_argument(
        "--mode",
        required=True,
        help="0 TEST COMS , 1 PERS HUA , 2 MANUAL CONTROL 3 PREFERENCES BASED",
    )
    argParser.add_argument("--ini", default="example.ini", help="MC ini file path")
    argParser.add_argument(
        "--profiles", default="drivers.yaml", help="Drivers info and models"
    )
    ...
    incoming_queue = Queue()
    reciever = UDP_Reciever(
        sensors_address["ip"], sensors_address["port"], incoming_queue, bytes_converter
    )
    transmiter = UDP_Transmitter(mc_address["ip"], mc_address["port"], bytes_converter)

    try:
        reciever_process = Process(
            target=data_reciever_service, args=(reciever,), daemon=False
        )
